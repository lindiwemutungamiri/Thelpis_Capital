<?php

require "database_connect.php";
$errors = "";
$passError = "";
$processError = "";

if(isset($_POST['userRegister'])){
    $fullName = $_POST['fullName'];
    $userMail = $_POST['userMail'];
    $pass1 = $_POST['pass1'];
    $pass2 = $_POST['pass2'];

    $checkEmail = "SELECT * FROM admin WHERE Admin_Email = '$userMail'";
    // $checkName = "SELECT * FROM user_login WHERE Full_Name = '$fullName'";
    $execMailCheck = mysqli_query($conn,$checkEmail);
    // $execNameCheck = mysqli_query($conn,$checkName);

    if($execMailCheck && $execMailCheck->num_rows > 0){
        $errors = "Email Already Exists";
    }else{
        if($pass1 == $pass2){
            $pass2 = md5($pass1);
            $insertQuery = "INSERT INTO admin VALUES(0,'$userMail','$fullName', '$pass2')";
            $execInsert = mysqli_query($conn,$insertQuery);
            if($execInsert){
                header('location:adminLoginPage.php');
            }else{
                $processError = "DATABASE INACESSIBLE";
            }
        }else{
            $passError = "Passwords Do Not Match";
        }
    }
}

?>